...
| **アンカー 4 台以上での 3D 測位**（台数は登録テーブルの長さで決まる。上限 8） | **実機確認済み**（2026-09-04〜05、アンカー4台・周期35ms・各アンカー成功率99.6〜100%。詳細は [`docs/HANDOFF.md`](docs/HANDOFF.md) §1） |
...
| **シリアルコンソールによる実行時設定**（アンカーのアドレス・座標を NVS〈ESP32 の不揮発設定ストレージ〉に保存。再ビルド不要） | **実機確認済み**（Wi-Fi 設定`wifi set/show`は2026-08-31、アンカー座標・アドレスの設定`anchor set`/`addr set`+NVS保存は2026-09-02・09-04） |
...
| 範囲 | 1 対 1 の測距まで | **測距・屋内3次元測位（ともに実機確認済み）+ 自動測量（実装済み・無線側未実装／実機未検証）** |
