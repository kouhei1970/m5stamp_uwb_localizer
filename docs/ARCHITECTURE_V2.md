...
- 統計・コンソール・LED などの補助処理が電波を扱うループと同じタスクにあり、補助処理のために
...
1. **アンカー**: 受信機を常時 ON にし、来たフレームの種類で分岐するステートマシンにする。
   電波を扱うタスクと補助処理（統計出力・コンソール・LED）のタスクを分ける（ESP32-S3 は 2 コア、FreeRTOS 1 kHz）。
...
## 8. StampFly 向けに機種固定で尖らせる設計（2026-08-30 深夜、ユーザー方針）

StampFly は機種固定（ESP32-S3 + M5Stamp UWB + 搭載センサ固定）なので、汎用の推定器を持ち込むのではなく、
この機体とこのチップの性質に合わせてアルゴリズムを絞り込み、性能で差を出す。搭載センサは M5Stack の公表仕様で
BMI270（IMU）、BMM150（磁気）、BMP280（気圧）、VL53L3CX（ToF、下向き・前向き）、PMW3901（光学フロー）——
**実装前に実機で要確認**。

### 8.1 使える「固定の事実」

1. UWB の測定時刻はチップのタイムスタンプ（RMARKER、≈15.65 ps 分解能）で分かり、交換の所要時間はこちらのプリセットで
   決定論的（DS 850 kbps/256 で Poll→結果 ≈8 ms、測定の実効時刻はその中点）。
2. アンカーの台数・配置は固定で事前に分かる（アンカー表）。アンカーごとの系統誤差（アンテナ遅延の個体差）も固定。
3. 高度は ToF（下向き）と気圧で別に取れる → UWB は水平 2 次元 + 冗長な z として使える（GDOP が改善）。
4. 受信品質の指標（受信電力 RSL、第一パス電力 FP、CIA の診断）が距離 1 本ごとに取れる。
5. ESP32-S3 は 2 コア・単精度 FPU。コア 1 を電波 + 推定器、コア 0 を飛行制御に固定できる。
...
| S3 | **アンカーごとの受信品質で R を変える**（FP−RSL 差が大きい = 非見通しの疑い → 分散を増やす、または棄却） | 4 | 外れ値（+0.3〜1 m）を検定に頼らず前段で弱める | twr の rsl/fp ログと実測の外れ値の相関を先に確認 |
| S4 | **アンカーごとのバイアス状態**（5 個、ゆっくり変化）を同時推定 | 2 | アンテナ遅延の個体差（cm 級）を飛行中に吸収 | 可観測性の確認（静止では不可観測 → 移動時のみ更新） |
| S5 | **能動測距**（次に測るアンカーを $\mathbf{u}_i^\top P \mathbf{u}_i$ が最大のものにする。欠測が続くアンカーは頻度を落とす） | 2, 5 | 同じ周期で誤差減、欠測アンカーの再試行の無駄が消える | シミュレーション → `RangingScheduler` の順序決定を差し替え |
| S6 | **一斉測距プロトコル**（Poll 1 回の放送 → アンカーがスロットで Response → Final 1 回 → Result をスロットで） | 1, 2 | 5 台で 1 周 45 ms → ≈18 ms（周 55 Hz、距離 275 本/s） | 2 台でスロット衝突なしを確認 → 5 台。アンカー基板の追加が必要 |
| S7 | 高度は ToF + 気圧、UWB は水平主体（z は弱い重み） | 3 | 天井・床のアンカー配置の制約が緩む、GDOP 改善 | 実機の ToF の有効範囲（数 m）を確認 |
| S8 | 単精度・Joseph 形式の共分散更新、コア 1 固定 | 5 | 400 Hz 予測 + 110 Hz スカラー更新で CPU 20% 以下（目標） | 実機で計測 |

### 8.3 進め方

1. まず S1 + S2 + S5 をシミュレーションで固め（同一乱数での対差、反証テスト付き）、`components/uwb_loc` に
   ホストテスト付きで実装する。基板 2 台で検証できる範囲。
2. S3・S4 は実測データ（rsl/fp と外れ値の関係、アンカーごとのバイアス）を先に採ってから。
3. S6 はアンカー基板が 2 台以上になった時点で。v2 のアンカーはステートマシンなのでスロット応答の追加は小さい。
4. 出力は「距離ごとに更新される状態」を飛行制御のレート（400 Hz）で読む形（§3.1 の `latestQueue`）。

PX4 EKF2 の遅延融合ホライズンは、遅れが既知で一定という UWB の性質（8.1-1）を使えば S1 の一次補正で置き換えられる。
これが「汎用でない」最初の差になる。

### 8.4 S1 / S2 / S5 の数値実験の結果（2026-08-30 深夜、5 系列、条件は §6 の数値実験と同じ測距モデル）

| ケース | 基準（逐次・等速） | S1（遅れ一次補正） | S2（IMU 密結合） | S2+S1 |
|---|---|---|---|---|
| 静止 RMS 3D | 3.09 cm | 3.14 | **1.47** | 1.47 |
| 円 1 m/s | 3.13（実効遅れ 4.0 ms） | 3.15（0.2 ms） | 1.50 | **1.45** |
| 円 2 m/s | 21.8（5 系列中 1 で発散） | 21.8 | **1.73（発散なし）** | **1.46** |
| 急加減速 3 m/s² | 8.42（>10 cm が 20%） | 8.45 | 3.99（2%） | 3.96 |

- S1 は厳密な巻き戻し方式と 0.05 cm 以内で一致（一次補正で十分）。遅れは 4 ms → 0.2 ms に消えるが、距離雑音 2 cm の床に
  対して RMS は 0.02〜0.05 cm しか下がらない（二乗和で効くため）。価値は遅れの除去と、S2 と組んだときの進行方向誤差（1.17 → 0.70 cm）。
- S2 が支配的: 全ケースで誤差半減、2 m/s の旋回での一時発散が消える。ただし姿勢既知・加速度計雑音 0.05 m/s²（機体振動を含まない）の前提。
- S5（能動測距）: 1 台無応答のとき、そこへの試行が 40% → 1.8%、RMS 1.75 → 1.46 cm。全台正常でも悪化しない（1.48 → 1.39 cm）。
- 判断: 実装順は **S2 → S1 → S5**。S2 は実機の IMU 雑音（モータ振動込み）で再評価が必要。
