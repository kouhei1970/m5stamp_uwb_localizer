...
`components/qm33120w_sdk/dw3720/dw3720_device.c:967-974` で、OTP（One-Time
...
1. **PLL 粗調整コードを工場値（この個体は 0x23）に固定する方法を確立
   する。**
   (a) 生レジスタで `PLL_COARSE_CODE` を書き、`PLL_CAL` の
   `PLL_USE_OLD`（0x2）/`PLL_TUNE_OVR`（0x4）ビットを立てて IDLE_RC→IDLE
   で再ロックし、読み戻しとロックフラグを確認する。成功率が 40% 以上に
   跳ねるかを見る。
   (b) だめなら init 前に `dwt_setpllcaltemperature()` 等で較正へ渡す
   温度入力を変えて挙動を見る。
   (c) DW3000 User Manual / Qorvo フォーラムで PLL 粗調整コードの温度
   依存と固定方法を確認する。
2. 冷起動の再現性をあと 2〜3 回（較正ダンプ付き）取り、0x23↔良
   （40% 台以上）、0x24↔悪（20% 以下）の対応を n≥3 で確定する。アンカー
   側も冷起動し、2 台とも 0x23 のときの成功率（100% 近くまで行くか）を
   見る。
3. 純正ファームでも冷起動 1 回（両機）を採る（DS-TWR なので
   0.7³≈35% 程度が期待値）。
4. 恒久対策の候補: 起動時の較正温度に応じてコードを補正・固定する、
   または温度が低いうちに較正した結果を保持する（sleep/wake の
   `DWT_LOADDGC` 等の kick と同様の仕組みが DW3000 にあるか調査する）。
5. 当座の回避策: 電源投入前に冷ます、起動直後の `cal:` 行で 0x24 なら
