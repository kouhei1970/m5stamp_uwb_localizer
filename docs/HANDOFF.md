...
- DW3000 は送信バースト時に 200〜250 mA の電流ピークを引く。Qorvo のレイアウト指針は
...
0. **ベース劣化の切り分け（RF／電源）**: (a) 送信バースト中のモジュール 3.3 V を
   オシロで見る（数十 mV 以上の落ち込みがあれば、470 µF 級のコンデンサを直近に足して
   再測定）、(b) `ipatov_power` を Qorvo フォーラムの正常値（20000〜30000）と同じ量か
   確認したうえで比較、(c) 別ロットのモジュールを 1 組入手して同じ試験、(d) スペアナで
   送信スペクトルとレベル。
1. **恒久対策の実装（起動ごとの振れ対策）**: `uwb::Qm33120::begin()` の `dwt_configure()` 直後に、
