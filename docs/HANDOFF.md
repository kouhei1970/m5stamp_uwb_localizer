...
距離掃引と送信電力 A/B は上の節で実施済み。残っているのは以下。

- **送信スペクトルを外部計測器で見る**（スペクトラムアナライザ、または
  広帯域検波器）。既定 vs 0x00000000 で出力が理論どおり ≈30 dB 変わるか、
  パルスの帯域が ch9（中心 7987.2 MHz ±250 MHz）に収まっているか。
  ソフトウェア経由の間接推定では決着しない「送信飽和説」を直接検証する
  唯一の手段。
- **pgDelay（パルス生成器の遅延。送信パルスの帯域を決めるパラメータ）を
  掃引する。** `PhyConfig::pgDelay`（既定 0x34、
  `components/uwb_qm33120/include/uwb_qm33120_types.hpp:322`）を
  0x28〜0x3f で振って成功率を見る。`dwt_calcpgcount(0x34)`
  （`components/qm33120w_sdk/deca_device_api.h:3146`）の値を 2 台で
  読んで記録すること。twr で回せるように Kconfig `UWB_TWR_DIAG_PGDELAY`
  を足すこと（今回は未実装）。
- **アンカー側のログも同時に採る。** ポールフレーム受信時の `rsl_dbm` と
  失敗種別を見て、往復のどちらのフレーム（ポール／レスポンス）が落ちて
  いるかを切り分ける。今回はタグ側ログしか見ていない。
- **850 kbps 構成**（`CONFIG_UWB_TWR_DIAG_PHY_850K=y`）でも距離掃引を
  最低 1 回行い、成功率が距離（受信電力）に依存しないままかを確認する。
- **候補 (a)（広帯域の妨害波／雑音の底）は今回も未確認のまま。** rsl
  では判定できないので、外部計測器で受信帯域の雑音そのものを見るか、
  この候補は保留にする。
- **計器の注意（追加）。** `rsl_dbm` は D（DGC 利得段）の 6 dB 刻みでしか
  動かず、2.13 m 以遠のように受信機が最大利得（D=0）に張り付く条件では
  −81 dBm 付近に頭打ちになる。1 dB 単位の細かい比較には `rsl_dbm` を
  使わないこと。細かく見たいときは `ipatov_power`（C）を D で正規化する
  か、外部計測器を使うこと。
- **ch5 での比較は不可（このモジュールは ch9 専用）。** 2026-08-29 に
  ユーザーから「このUWBはch９専用です」と明示された。M5Stamp UWB Module
  のアンテナ／RF 前段は ch9（中心 7987.2 MHz）向けで、ch5（6489.6 MHz）は
  使えない。前々回コミット（`f4d809d`）の Next steps にあった「ch5 を
  試す」は無効。同日に一度 twr へチャネル選択の Kconfig を足したが、この
  事実を受けて取り消した（コミットしていない）。「妨害が帯域局在か」の
  切り分けは、チャネル切替ではなく別の手段（例: 送信を止めた状態で
  受信機の雑音指標を距離・時刻・場所を変えて採る）で行うこと。
- 今回の実機ログの生データも一時的な作業領域（スクラッチ領域）にしか
  残っておらず、セッションが終われば失われる。**このファイルに書き写した
  数値が本記録である。** 次に実機を触るときは、新しく採ったログの要点を
  都度この §0-B に反映すること。
