...
- **ch5 での比較は不可（このモジュールは ch9 専用）。** 2026-08-29 にユーザーから
  「このUWBはch９専用です」と明示された。M5Stamp UWB Module のアンテナ／RF 前段は
  ch9（中心 7987.2 MHz）向けで、ch5（6489.6 MHz）は使えない。前回コミット
  （`f4d809d`）の Next steps にあった「ch5 を試す」は無効。同日に一度 twr へ
  チャネル選択の Kconfig を足したが、この事実を受けて取り消した（コミットしていない）。
  「妨害が帯域局在か」の切り分けは、チャネル切替ではなく別の手段（例: 送信を止めた
  状態で受信機の雑音指標を距離・時刻・場所を変えて採る）で行うこと。
- 続けて **距離を広げながら `rsl_dbm` と成功率を採り、実効感度を逆算する**
  （現状は「−70 dBm で 5 割落ちる」の 1 点しかない。感度カーブを描いて、仕様値
  −102 dBm（850 kbps）との差 30 dB がどの距離・どの受信電力で効いているかを見る）。
- 今回の実機ログの生データは一時的な作業領域（スクラッチ領域）にしか残っておらず、
  セッションが終われば失われる。**このファイルに書き写した数値が本記録である。**
  次に実機を触るときは、新しく採ったログの要点を都度この §0-B に反映すること。
