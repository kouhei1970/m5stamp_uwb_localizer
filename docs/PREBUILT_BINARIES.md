...
また、実機で動作確認済みなのは**タグ 1 + アンカー 1 の 1 対 1 測距とブラウザ
ダッシュボードまで**で、アンカー 4 台以上での 3D 測位はまだ実機で動かしていません
（現状の詳細は [`../README.md`](../README.md)）。実機確認はローカルビルドで行っており、
**ビルド済みバイナリそのものでの動作確認はしていません**。
