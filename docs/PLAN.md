...
| 論点 | 結論 |
|---|---|
| M5Stamp UWB Module with FPC を SPI で使えるか | **使える。というより SPI しか無い**（Qorvo QM33120W 直結、AT コマンド無し） |
...
1. **uwb_localizer にはチップドライバも測距シーケンスも無い。**
   ...
   → M5Stamp UWB Module with FPC 用のレンジング層は**まるごと新規**。ただしゼロからではなく
...
| D2 | タグ側ファームウェア | Stamp S3 / AtomS3 + M5Stamp UWB Module with FPC で測距→測位まで完結 |
...
手持ちは M5Stamp UWB Module with FPC ×6 なので、当面の検証構成は**タグ1 + アンカー5**。
...
**本リポジトリは ESP32-S3 + M5Stack M5Stamp UWB Module with FPC 専用。プラットフォーム最適化を積極的に行ってよい。**
...
- **必要ハード: M5Stamp UWB Module with FPC ×2 + ホストボード ×2**
...
- **受入条件: アンカー4台の既知配置で3D位置が出ること、
  ...
  （静止位置の誤差 0.3m 程度を目標）
- **必要ハード: M5Stamp UWB Module with FPC ×6（確保済み） + ホストボード（台数要確認。
  4台あれば受入条件の下限は検証できる）**
...
- 保有ハード: **M5Stamp UWB Module with FPC / ホストボード ともに4台以上** → Phase 4（3D測位）まで実機検証可能
