...
- 接続手段（下記）
- **3.3V 電源**（公式仕様 3.3V。パッド 2 は QM33120W にチップ直結のため **5V は入れないこと**（絶対最大定格 4.0V）。詳細は [`SOLDER_PADS.md` §5.4](SOLDER_PADS.md)）
- ESP-IDF **v5.5.2**
...
信号名は同一ですが、**パッド番号と FPC ピン番号の並びは異なります**（例: FPC の 1・2 番が 3V3、7・11 番が GND。対応表は `SOLDER_PADS.md` §2・§5.5）。FPC ケーブルで繋ぐときは FPC 側の番号で配線してください。
...
### モジュール側 12 本（**パッド番号と FPC 番号は別体系**）

> **【訂正 2026-08-22】** 本節の見出しは以前「FPC ピン番号 = 半田パッド番号。固定」と
> 書かれていたが**誤り**だった。表もパッド番号だけを載せていた。
...
|---|:-:|:-:|---|
| GND | 1 | 7 | グランド |
| **VCC_3V3** | **2** | **1, 2** | **公式 3.3V。チップ直結、絶対最大定格 4.0V。5V 不可** |
| DW_WAKEUP | 3 | 5 | ウェイクアップ制御 |
| DW_IRQ | 4 | 4 | 割り込み出力 |
| DW_GP7 | 5 | 3 | 予備。配線不要 |
| DW_RSTn | 6 | 6 | リセット（負論理） |
| DW_CDO | 7 | 8 | SPI **MISO** |
...
- `DW_RSTn` はホストが能動的に High へ駆動しません。リセット時のみ Low に引き、
  それ以外は入力（ハイインピーダンス）にしています。High 側は**データシート上
  IC 内蔵の POR 回路が駆動する設計**で、**外部からのプルアップは禁止**
  （"Must not be pulled high by the external source"）されているため、
  ハーネス上にプルアップを追加しないでください（2026-08-21 訂正。以前の版は
  保険として推奨していましたが誤りでした）。疎通が取れないときは
  wakeup シーケンスや配線長・GND の戻り経路を疑ってください。
...
| L1 は OK だが L2 が失敗 | `dwt_probe()` の wakeup シーケンス／SPI の再現性を疑う。RSTn へのプルアップは DS で禁止（Must not be pulled high）なので付けない |
| **DEV_ID は読めるのに `begin()` が `CONFIG_FAILED`**（書き込み直後の再起動で発生、USB 抜き差しで直る） | RST 未配線で UWB チップが前回状態を引きずる（外部報告で確認された症状）。本リポジトリは `begin()` でソフトリセットするので起きないはず。起きたら `Config::soft_reset_on_begin` が true か、`pin_rst` 配線時はリセットパルスが出ているか確認。最終手段は電源断 |
