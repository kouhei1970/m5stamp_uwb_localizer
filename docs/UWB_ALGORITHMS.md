...
> **この文書について（本リポジトリ向け移植版）**
>
> 原典は [kouhei1970/uwb_localizer](https://github.com/kouhei1970/uwb_localizer) の
> `docs/UWB_ALGORITHMS.md`（MIT License, Copyright (c) 2026 Kouhei Ito）であり、
> 本文書は本リポジトリ（m5stamp_uwb_localizer）向けに改訂した移植版である。
>
> 本リポジトリの測位ソルバ `components/uwb_loc/` は、上流 uwb_localizer の C 版ソルバを
> **無改造で vendoring** したもの（取り込み元コミット `8d0edc057ed05cf6b4af91df329999fe2343f515`。
> 詳細は [`THIRD_PARTY_LICENSES.md`](../THIRD_PARTY_LICENSES.md)）である。
> **したがって、この文書の数式・導出はそのまま本リポジトリの実装の説明になる。**
> 改訂したのは「本リポジトリのどこに実装があるか」「本リポジトリで何が有効か」を示す注記の追加だけであり、
> 原典の数式・導出・記号の定義・章立ての骨格は一切変更していない。
>
> **読む順路**: [`docs/UWB_PRIMER.md`](UWB_PRIMER.md)（物理と測距の原理）
> → 本書（測位の数理）→ [`docs/ANCHOR_PLACEMENT.md`](ANCHOR_PLACEMENT.md)（アンカー配置の指針）。
>
...
>
> **数値例・実測値について**: 本文中の「実測」は、断りが無い限り**上流 uwb_localizer 側の検証
> （ホスト PC 上のテスト・シミュレーション。§11 の比較表は乱数シード違いの合成観測列を使っている）
> によるもの**であり、本リポジトリ（m5stamp_uwb_localizer）の実機で測ったものではない。本リポジトリはまだ
> 実機での測距・測位を一度も確認していない（[`README.md`](../README.md) の「現状」参照）。
> 数値そのものは原典のまま変更していない。
...
| [§5](#5-lv2--ロバスト化) Lv2 ロバスト化 | Huber + 片側損失 | `components/uwb_loc/src/uwb_nls.c`（`uwb_solve_lv2()`） |
| [§6](#6-lv3--密結合-ekf) Lv3 密結合 EKF | カルマンフィルタ | `components/uwb_loc/src/uwb_ekf.c`（`uwb_ekf_init()` / `uwb_ekf_predict()` / `uwb_ekf_update()`） |
| [§7](#7-tdoa-の-chan-法) TDoA の Chan 法 | 背景知識。**本リポジトリでは未実装**（TDoA 自体を使わないため） | — |
| [§8](#8-gdop-と-crlb--配置を数字で評価する) GDOP・CRLB | 配置評価 | `components/uwb_loc/src/uwb_model.c`（`uwb_gdop_at()` / `uwb_crlb_at()` / `uwb_anchors_coplanar()`） |
| [§9](#9-アンカーの自己測量) 自己測量（MDS） | アンカー座標の自動復元 | `components/uwb_loc/` には**対応する関数が無い**（上流 Python 版 `calibration.py:self_survey` の C 移植は無い）。本リポジトリでは同じ考え方（古典的 MDS + Gauss-Newton 精密化）を `components/uwb_survey/` が ESP-NOW 前提で独自に実装している（`docs/SURVEY_SPEC.md`）。コードの系統はこの文書の §9 とは別 |
| [§10](#10-アンテナ遅延の推定) アンテナ遅延の推定 | 遅延の分離推定 | `components/uwb_loc/` には**対応する関数が無い**。本リポジトリでは手動校正手順（`docs/GETTING_STARTED.md` §9「アンテナ遅延の校正」）と、`components/uwb_survey/` の自己測量が推定する共通遅延 δ（`docs/SURVEY_SPEC.md`）の 2 経路がある |

（各節冒頭の「実装:」注記は原典 (Python 版) のファイル参照のまま残してある。本リポジトリでの対応は上表を見ること。）
...
**初出で必ず展開する**が、まとめて置いておく。本リポジトリの用語集
[`docs/GLOSSARY.md`](GLOSSARY.md) の表記ルール（「正式名称（英語）＝日本語の意味」を初出で添える）とも揃えてある。
...
> **注記（本リポジトリでの実装について）**: 以下、各節冒頭の「実装:」は原典
> （上流 uwb_localizer の Python パッケージ `uwb_loc`）でのファイル参照であり、
> **本リポジトリには Python API は含まれない**（本リポジトリはファームウェアであり、C/C++ のみ）。
> 数式の説明に必要な範囲でこの後もこの形の参照を残すが、擬似コードとして読むこと。
> 本リポジトリでの実装（C 版）は、この文書冒頭の「この文書と本リポジトリの実装の対応」表を見ること。
...
「これが距離なのか角度なのか」を一切区別しない。だから観測種別を増やしても
ソルバを直さなくてよい。

### 1.1 距離 (TWR)

> **本リポジトリでの位置づけ**: TWR による距離観測が**本リポジトリの本線**（唯一実装している測距方式。
> §1.2/§1.3 の TDoA・AoA/PDoA は未使用）。測距そのものを行っているのは
> `components/uwb_qm33120/src/uwb_qm33120_twr.cpp` の `requestRange()`（SS-TWR）/
> `requestDSRange()`（DS-TWR）。以下の観測モデルとヤコビアンは、そこで得た測距値を
> 測位ソルバ（`components/uwb_loc/`）に渡す際にそのまま使われる。

**TWR** (Two-Way Ranging, 双方向測距) は、電波を往復させてその時間から
距離を求める方式。片道の時刻を測る方式と違い、**送信側と受信側の時計が
合っている必要が無い**のが利点で、UWB モジュールの多くがこれを使う。

観測モデルは単純に「タグとアンカーの距離」。
...
> **本リポジトリでは未使用**（`components/uwb_qm33120` は TWR のみを実装しており、
> TDoA に要るアンカー間の時計同期は無い）。背景知識として原典のまま残す。
...
> **本リポジトリでは未使用**（`components/uwb_qm33120` は TWR による距離測定のみを実装しており、
> AoA/PDoA に要る複数アンテナ・位相差の測定機能は無い）。背景知識として原典のまま残す。
...
そのまま $w_i = 1/\sigma_i^2$ を使うと、遠いアンカーを過大評価する結果になる。
...
> **本リポジトリの `SolverLevel` には Lv1 が無い**（`components/uwb_ranging/include/uwb_ranging_types.hpp:40-42`
> の `enum class SolverLevel` は `Lv0` / `Lv2` / `Lv3` の 3 つのみで、`Lv1` を含まない）。
> Lv1（本節の重み付き非線形最小二乗）は Lv2（[§5](#5-lv2--ロバスト化)）の内側で使われる構成要素であり、
> 単体では公開していない。C 実装では `components/uwb_loc/src/uwb_nls.c` の `uwb_solve_lv1()` と
> `uwb_solve_lv2()` はどちらも共通の内部関数 `solve_iterative()`（ロバスト化の有無を切り替えるフラグ違い）を
> 呼ぶだけの薄いラッパになっており、`uwb_solve_lv1()` 自体は `uwb_loc.h` が宣言していて C API としては
> 呼べる。ただし本リポジトリのファームウェア（`uwb_ranging` 層の `PositioningPipeline::solve()`）は
> `SolverLevel` しか受け取らないため、Lv1 を単独では呼び出せない。**Lv2 は Lv1 の上に立っている**ので、
> この節は削除せずそのまま残す。`docs/PERF_ANALYSIS.md` には Lv1（`uwb_solve_lv1`）の性能実測値が残っている。
> `docs/GLOSSARY.md` の「Lv0 / Lv2 / Lv3」の項も参照。
...
**位置だけ返すライブラリは現場で困る** — 「今の値をどれだけ信頼できるか」が
判断できないため。
...
**ここが実装で最も試行錯誤した箇所。** 当初の設計どおり常時動かしたら、
...
1. **起動条件をつける** — 標準化残差の RMS $\sqrt{\overline{t^2}}$ が 3 を超えたときだけ実行する
...
| $K$ | **カルマンゲイン**。「観測をどれだけ重視して state を動かすか」 |
...
- **立ち上げ** (bootstrap) は Lv2 の解と共分散を使う。速度は不明なので大きな事前分散から始める。
  ただし**測距 1 本では位置が決まらない** (球面 1 枚) ので、複数本たまるまで待つ。
  受理条件は次の 3 経路 (いずれか 1 つで可、優先順): (1) $d+2$ 本そろえば即座に
  立ち上げ、(2) $d+1$ 本そろい、かつそれが登録されている enabled なアンカーの
  全台数なら (これ以上待っても増えない)、待たずに立ち上げ、(3) $d+1$ 本そろって
  いて経路(2)に届かない (一部のアンカーが遮蔽されている) 場合、待ち始めてから
  `max_dt` 経てば妥協して立ち上げる。経路(2)が無いと、有効アンカーが $d+1$ 台
  しかない構成 (3 台×2D・4 台×3D など) では $d+2$ 本に永遠に届かず初期化でき
  ない (詳しくは §13.4)
...
> **本リポジトリでは未実装**（[§1.2](#12-距離差-tdoa) の注記のとおり、TDoA 自体を使わない構成のため）。
> `components/uwb_loc/` にも対応する C 関数は無い。背景知識として原典のまま残す。
...
警告無しに 1/2 の確率で高さが折り返るのがいちばん危ないため。
...
**動作の照合は、同じ観測列を Python 版と C 版に入力して `Fix` を突き合わせる**のが
いちばん早い。上流の `third_party/uwb_localizer/tools/crossval.py`（**本リポジトリには Python ツールは無い**）が
これを自動でやっており、
...
## 13. 測距1本ごとの逐次EKF観測更新（ファームウェアでの配線）

§6.3 で述べた「観測を1本ずつスカラーで処理する」設計は、`uwb_ekf_update()`
（`components/uwb_loc/src/uwb_ekf.c`）自体は最初からこの形（`meas[]` を
先頭から1本ずつループし、逐次スカラー更新する）で実装されていた。しかし
`uwb::RangingService`（`components/uwb_ranging`、タグファーム側の測距・測位
専用タスク）は当初、**1周期ぶんの測距が全部終わってから**、その周期の
全サンプルをまとめて `updateEkf(tS, samples, n)` に1回だけ渡していた
（`tS` は周期開始時刻1個だけ）。

### 13.1 なぜ「まとめて1回」だと精度が落ちるか

`uwb::RangingScheduler::runCycle()`（`components/uwb_ranging/src/
uwb_ranging_scheduler.cpp`）はアンカー登録テーブルを順に1台ずつポーリング
する。台数が $N$、1台あたりの往復（再試行込み）に $\tau$ かかるとすると、
1周期の所要時間は $N\tau$ 程度になり、$k$ 番目（$0$-indexed）のアンカーは
周期開始からおよそ $k\tau$ だけ遅れた時刻に測られる。`RangingSample::t_us`
（`uwb_ranging_types.hpp`）にはこの**実際の測距時刻**が個別に入っている。

タグが速度 $v$ で動いているとき、周期開始時刻を全アンカー共通の観測時刻だと
みなして一括更新すると、後の方に測ったアンカーほど「本当は $v k\tau$ だけ
先に進んだ位置で測ったのに、周期開始時点の位置で測ったことにされる」歪みが
乗る（実測: 1周32msで5台を順に測ると同一周でも台ごとに最大32msの差があり、
1m/sなら32mmの歪みになる。`docs/HANDOFF.md` §5 F）。EKFの内部ではこれは
単純な時刻オフセットではなく、**同一エポックの複数観測が実際には異なる
真の位置に対応しているのに、モデルが「同時刻の観測」として線形化する
こと**による幾何的な歪みとして現れ、等速直線運動でも定常的な位置バイアス
（ラグ）を生む。

### 13.2 対策: 測距1本ごとのフック (`RangingSampleHook`)

`RangingScheduler` に測距1本が成功するたびに呼ばれるフックを追加した
（`uwb_ranging_scheduler.hpp` の `RangingSampleHook` 型、
`RangingScheduler::setSampleHook()`）。`std::function` ではなく、本リポジトリの
他コールバック（`uwb_net.hpp` の `StatusJsonFn`）と同じ「関数ポインタ +
`void*` コンテキスト」の作法にしてある。フックは `runCycle()` が
`rangeOne()` から `RangingSample::ok==true` を受け取った**直後、同じタスク
文脈で同期的に**呼ばれる（新しいタスクやキューは作らない — `uwb_ekf` は
スレッド安全でないため、測距を回すタスクの外へ非同期に持ち出さない設計を
崩さないため。§6 冒頭注、`uwb_ranging_pipeline.hpp` の `PositioningPipeline`
コメント）。

`uwb::RangingService::onRangingSample()`（`uwb_ranging_service.cpp`）が
このフック本体で、測距に成功したサンプル1本ごとに

$$
\texttt{updateEkf}\bigl(\text{sample.t\_us} / 10^6,\ \&\text{sample},\ n{=}1\bigr)
$$

を呼ぶ。§6.3 の「観測を1本ずつ処理する」性質（$H$ が行ベクトル、$S$ が
スカラー）を、**タイムスタンプまで含めて**ファームウェアの呼び出し方に
反映した形になる。周期末尾でまとめて呼ぶ経路は廃止した
（同じ測距をEKFへ二重に入れないため）。1周期に1本も測距が成功しなかった
場合は、EKFの内部時刻はそのまま（次に成功した実測時刻からの $dt$ で
正しく予測が繋がる）にして、その周期のCycleResult上は「新しい推定は無い」
ことを `ok=false` で示す（Lv0/Lv2が欠測時に `solvable=false, ok=false` を
返すのと意味論を揃えた）。詳しい配線とロック規律は
`uwb_ranging_service.hpp` 冒頭コメント（タスク・キュー構成、
`tableMutex()`）を参照。

### 13.3 効果の実測（ホスト側合成データ検証）

`tests/host/pipeline/test_pipeline.cpp` のシナリオ26は、同一の合成軌道
（アンカー5台立体配置、等速直線0.75 m/s、1台あたり測距間隔32ms）に対し
「周期末尾で一括更新」と「測距1本ごとに実測定時刻でn=1の逐次更新」を
それぞれ100周期回して、真値との位置誤差RMSを比較している。

| 方式 | 位置誤差RMS | 最大誤差 |
|---|---|---|
| 一括更新（周期開始時刻1個にまとめる、旧実装相当） | 0.095 m | 0.118 m |
| 逐次更新（測距1本ごとに実測定時刻でn=1、本実装） | 0.00000 m | 0.00001 m |

（静止タグでは両方式とも数学的に等価な最終位置に収束することをシナリオ25で
別途確認している — §13.1の歪みは「動いている間に台ごとに時刻がずれる」こと
自体が原因なので、静止タグでは現れない。）

### 13.4 bootstrap 受理条件の修正（有効アンカーが d+1 台の構成で永遠に初期化できない不具合）

§6.4 で述べた bootstrap の「$d+2$ 本たまるまで待つ」実装には、待っても
$d+2$ 本に届かない構成 (有効アンカーが $d+1$ 台しかない、3 台×2D・4 台×3D
など) で永遠に初期化できない不具合があった。

原因は「$d+2$ 本に届かないときの救済」の書き方にある。届いた観測は
`pending[]` に貯め、`max_dt` より古いものは毎回刈り込む。この刈り込みの
直後は必ず「一番古い pending の年齢 ≦ `max_dt`」になる（`max_dt` を超える
ものは刈り込みで消えているので当然）。ところが旧実装の救済条件は
「一番古い pending の年齢 ≧ `max_dt` になったら妥協する」という書き方だった
ため、直前の刈り込みの効果でこの条件がほぼ恒等的に成立しない（浮動小数の
等号一致を除く）**事実上機能しないコード**になっていた。有効アンカーが $d+1$ 台の
構成では $d+2$ 本の到達自体が原理的に不可能なので、救済も働かず、EKF が
永遠に `initialized` にならなかった。

修正後の受理条件（`components/uwb_loc/src/uwb_ekf.c` の `bootstrap()`）は
次の 3 経路 (優先順、いずれか 1 つで可):

```
m >= d+2                                     … 経路1: 理想本数が揃った。即立ち上げ
m >= d+1 かつ m >= 登録enabledアンカー台数   … 経路2: これ以上待っても増えない
m >= d+1 かつ「待ち始めてから max_dt 経過」  … 経路3: 一部遮蔽への救済
```

（$m$ = 直近で揃っている異なるアンカーの本数、$d$ = `cfg->dim`。）
経路2の「登録 enabled アンカー台数」は `uwb_config.anchors[i].enabled` を
直接数える（`plane_compute()` と同じ数え方）。経路3の「待ち始めた時刻」は
`uwb_ekf` 構造体の `boot_wait_t0`（pending が空から最初の 1 本を積んだ時刻。
刈り込みでは更新されず、pending が全部刈り込まれて空に戻ったときだけ
リセットされる）で追跡し、刈り込みで再計算される「一番古い pending の
年齢」とは切り離してある。これが経路3の救済を実際に機能させる肝になる。

経路2により、3 台×2D・4 台×3D といった $d+1$ 台構成は最初の1周期
（$d+1$ 本そろった時点）で即座に立ち上がる。経路3は、有効アンカーは
$d+2$ 台以上あるが一部が遮蔽されていて $m$ が $d+1$ 止まりの場合の救済で、
`tests/host/loc/test_uwb.c` の `test_ekf_bootstrap_rescues_with_one_anchor_occluded`
で検証している。3 台×2D の主構成での即時立ち上げは
`test_ekf_bootstraps_three_anchor_2d_one_cycle`（`tests/host/loc`）と
`scenario28_ekf_bootstrap_three_anchor_2d_mode`（`tests/host/pipeline`）で
それぞれ確認している。
