...
### 1.2 受信窓を開く締切
`dwt_setrxaftertxdelay(D)` は**自局の送信が終わってから** D 後に RX を開く。
一方 `responseTxDelayUus` (= R) は**相手局の RX タイムスタンプ（RMARKER）から** R 後に送る。

相手局のフレームの**プリアンブル先頭**が自局に届くのは、自局の送信終了を基準にして
...
送信側が「相手局のフレームを受信したと検出してから、遅延送信を予約し終える」までの時間。

| 待ち方 | 検出するまでの遅れ | SPI 数回 + 計算 | 合計 |
|---|---:|---:|---:|
...
タグから見て DWD のプリアンブル先頭が届くのは、自局の Final 送信終了を基準に
...
開放された時刻」を起点に動作するタイマーで、上の表の「開く時刻」から「プリアンブル
...
**折返しに要る時間（ポーリング、§1.3 と同じ内訳）**: 検出するまでの遅れ ≤ 1 ms
...
- `finalRxAfterResponseTxDelayUus` は `dwt_setrxaftertxdelay()` に渡る。Qorvo
  ...
  送信が**完了した後**の遅延）と説明しており、基準は**自局（アンカー）の Response
  送信完了時刻**である。
...
**旧版の相手局からのフレームは「不一致」として捨てられ、警告すら出ない**
...
警告は**同じ相手局について 1 回だけ**出す（毎周期 5 台分の警告でログが埋まるのを防ぐ）。
...
`init()` が IRQ 待ちを打ち切ってポーリングへ切り替えるのは、次の**いずれか**に該当するときである。
...
3. **（2026-08-28 追加）`Qm33120::verifyIrqLine()` による実測で IRQ 線が動作していないと判定された**
...
（実際には全ての待ちが 1ms タイムアウトへ表面化せずに劣化していた）。
...
- **段階0（前処理）**: `dwt_forcetrxoff()` + `dwt_writesysstatuslo(ALL_RX_TO |
  ...
  「エッジが来ない」を判定することになり、線が動作していても偽陰性になる。
...
- **段階2（事象確認、最大 50 ms）**: `dwt_setrxtimeout(2000 UUS)`（約 2.05 ms）
  ...
  に含まれるので、線が動作していれば必ずエッジが1回来るはずである。SYS_STATUS
  に事象（RXFTO・RX エラー・RX 成功のいずれか）が立っているのにエッジが来
  なければ、線は動作していないと判定して false を返す。事象自体が立たなかった
...
**本節の記述はビルド確認のみで実機未検証。** IRQ 線が実際に動作している個体で
...
警告するだけ。根拠は「相手局と一致していることが要件なので、片側が勝手に変えた
ら §0 の破綻そのものになる」。警告の例:
...
2. **締切を割った遅延送信は SDK が送信ごと取り消す。**
   ...
   れない。相手局からは「RXFTO のみが立ち、RXPRD（Preamble Detected、プリアン
...
4. **万一片側だけ降格しても、§3 の不一致検出が働く。** 実際に適用した種別
   （降格後の値）は Poll/Response フレームに載って相手局へ伝わるので、片側だけ
