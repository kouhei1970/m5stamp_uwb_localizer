...
| [JA_ENGINEERING_TERMS.md](JA_ENGINEERING_TERMS.md) | 言葉遣いの辞書。直訳語・業界スラング・擬人化の「使わない語 → 言い換え」一覧と、残してよい定訳。文書を書く前に引く |
...
7. **直訳語・業界スラング・擬人化を使わない。** 装置やソフトウェアが主語のときは、
   比喩ではなく実際の動作を書く。使わない語と言い換えの一覧は
   [`JA_ENGINEERING_TERMS.md`](JA_ENGINEERING_TERMS.md)（言葉遣いの辞書。正本）にある。
   文書を書く・直す前に必ず一度引くこと。