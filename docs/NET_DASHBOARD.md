...
| **測位図**（旧称: 平面図・位置図） | 登録済みアンカーの座標、現在の測距値を半径とする円、位置解の軌跡（最大 200 点）、最新の推定位置。2D/3D の切替と「拡大」ボタン（ブラウザ全体に広げる。Esc または「戻る」で解除）付き。**アンカー 1 台のときは点+測距円までで、位置解の十字マークと軌跡はアンカー 3 台以上で測位が成立して初めて表示される** |
| **コンソール** | §5 参照 |
...
**症状**: ダッシュボードをしばらく開いていると更新が止まる、ヘッダの受信レート
