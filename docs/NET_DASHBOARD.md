...
| **平面図** | 登録済みアンカーの座標、現在の測距値を半径とする円、位置解の軌跡（最大 200 点）、最新の推定位置。**アンカー 1 台のときは点+測距円までで、位置解の十字マークと軌跡はアンカー 3 台以上で測位が成立して初めて表示される** |
| **コンソール** | §5 参照 |
...
- [x] ヘッダ・ノードカード・アンカー一覧・チャート・平面図が実データで更新されること（2026-08-31）
- [ ] 「記録開始」→「保存」で `.jsonl` が実際にダウンロードされ、中身が読めること
- [ ] ページ内コンソールから `anchor list` 等を打って応答が返ること
- [ ] 複数ブラウザタブを同時に開いても壊れないこと
...
| 種別 | 出どころ | 頻度 | 内容 |
|---|---|---|---|
| `anchors` | タグ | 起動時・表が変わるたび・`stats` と同じ周期（既定 1 秒）で再送 | 登録済みアンカーの一覧（座標・アンテナ遅延・有効/無効）。後から接続したブラウザにも座標を届けるための再送で、この行が無いと平面図が描けない |
