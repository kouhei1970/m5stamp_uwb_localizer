...
> 旧ファームのゾンビ接続です。** 2026-09-02 以降のファームでは、切断した
...
気づかず 50ms ごとに配信を続けます。すると相手から **RST**（接続の強制切断通知）
...
「切断しない」と応答してしまい、この接続は**ゾンビ**として残り続けます。ゾンビは
配信のたびに同じ失敗を繰り返す**ビジーループ**（休みなく同じ失敗を繰り返す状態。
実機で 26 秒間に 1.9MB の警告ログが出た例あり）を起こして全員への配信をカクつかせ、
さらに失敗のたびに自分の **LRU**（最も長く使われていない接続を追い出す仕組み）の
使用時刻だけが更新されるため、実際に見ている（受信専用で送信をしない）ブラウザの
方が「長く使っていない」と誤判定され、接続数の上限（6）に達すると**そちらが
追い出される**という逆転が起きていました。再読み込みのたびにゾンビが 1 つ増えます。
...
1 秒程度でゾンビが消え、機器ごとネットワークから消えた場合でもキープアライブに
より約 20 秒で片付きます。
...
- **598525c より前のファームには、PC を繋がない給電のみ（充電器/モバイルバッテリ、
  ...
  ループで Wi-Fi 起動タスクが飢餓していました）。**最新ファームでは修正済みです。**
