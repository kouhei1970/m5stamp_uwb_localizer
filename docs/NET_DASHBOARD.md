...
アンカーも同じ 20 秒でタイムアウトして自局の SoftAP へ切り替わり、
...
アンカーは 21 秒でいったん自局の SoftAP へフォールバックし、**55 秒後に
...
SSID を一度も設定していないアンカーは、`auto` では最初から自局の SoftAP を
...
> **同時には実行されません**（片方が実行中なら、もう片方は次のタイミングに回ります）。
...
> **`ws clients` が実際に開いているブラウザ数より多いまま減らない場合は、
> 旧ファームで切断されたのに残り続ける接続（いわゆるゾンビ接続）が残っています。**
> 2026-09-02 以降のファームでは、切断した
> ブラウザの接続が数秒〜約 20 秒で自動的に片付くようになりました
> （詳細は §4「ブラウザの更新が止まる問題」）。
...
| 要素 | 内容 |
|---|---|
| **上部バー**（画面上端に固定） | 機体名・役割チップ、ライブ／再生の切替、接続ピル（緑点=接続中・受信レート「◯行/秒・◯KB/秒」）、測位モードチップ（測距のみ／2D測位／3D測位。2D測位では固定zの値も併記）、記録開始・保存ボタン、配色モード切替ボタン |
| **KPI 帯** | 測位解の x/y/z（各60秒分のスパークライン付き）・周期・成功率（全アンカー合算）・残差RMS + GDOP。§4.2 |
| **ヒーロー行**（測位図 + アンカー一覧） | 画面の残り高さいっぱいに広がる2枚のパネル。§4.3・§4.4。**画面幅1100px未満では縦に並びます** |
| **チャート**（直近60秒、2×2） | 距離・成功率・周期・位置。§4.5 |
| **測距統計・位置ジッタ** | ブラウザ側で計算する統計表2つ + キャプション。§4.6 |
| **コンソール** | §5 参照 |
| **ノード**（折りたたみ、既定は閉） | 自機と全アンカーのカード。§4.7 |
| **生ログ**（折りたたみ、既定は閉） | 直近200行の生JSON。§4.8 |
| **フッター** | 機体構成の注記 |
| **再生バー**（画面下端に固定、再生モード時のみ表示） | §4.10 |

**ヒーロー行は画面の高さいっぱいまで広がります。** その高さは、そのときの
ビューポート（ブラウザの表示領域）の高さから、上部バー・KPI帯・余白・
（再生モードなら）再生バーの実測の高さを差し引いた値で、最小420pxまで
縮まり得ます。**画面幅が800px未満（縦向きのスマートフォン等）になると
この高さ制約自体を外し、内容なりの自然な高さに戻ります。**

### 4.2 KPI帯

測位解の x/y/z は、**EKF**（Extended Kalman Filter、拡張カルマンフィルタ。
運動モデルと観測値を逐次融合して状態を推定する手法。ここでは `fix.lv3`）に
よる解があればそれを優先し、無ければ通常解（`fix.p`、非線形最小二乗法に
よる解）を表示します（アンカー一覧・測位図・チャートの主表示も同じ優先順位
です）。**一方、「残差RMS」と「GDOP」は常に通常解（`fix`）側の値で、EKFが
あってもEKF側の値には切り替わりません。** 「残差RMS」は測位計算の残差
（各アンカーへ推定した距離と実測距離の食い違い）のRMS（Root Mean Square、
二乗平均平方根）、「GDOP」（Geometric Dilution of Precision、幾何学的精度
低下率。測距誤差が位置誤差へ何倍に拡大されるかを表す倍率で、小さいほど
良い）は `fix.gdop` の値です。

### 4.3 測位図（旧称: 平面図・位置図）

2D/3Dの切替タブ、「拡大」（ブラウザ全体に広げる。Esc または「戻る」で解除）、
「点群消去」「軌跡消去」ボタンを備えます（「軌跡消去」は直近 60 秒の軌跡だけを空にし、点群と印には触れません）。マーカー・軌跡はKPI帯（§4.2）と同じ優先順位
（EKF優先、無ければ通常解）で描き、軌跡は直近60秒分を薄い線で表示します。
点群（紫）は毎周期の位置解をそのまま積み上げたもので、「点群消去」を押す
まで消えません（上限2万点。超えると古い点から捨てます）。無効化された
アンカーは灰色のドットで表示します（2D/3D共通）。**アンカー1台のときは点＋
測距円までで、位置解の十字マークと軌跡は測位が成立して初めて表示されます**
（2D測位はアンカー3台以上、3D測位は4台以上が必要。ファームウェア側の要件で、
ページ側で台数を数えているわけではありません）。

**2D**: キャンバスの全面がグリッドで埋まります（キャンバスの実寸・スケール・
中心からワールド座標の可視範囲を直接計算するため、内容の分布に関わらず
グリッドが縁まで届きます）。目盛りの数字はキャンバス内側の縁近くに描かれ
ます。表示範囲は0.5m刻みのグリッドへ丸めた上で、内容がはみ出せば即座に
拡大し、逆に「表示範囲の70%未満」の状態が3秒以上続いたときだけゆっくり
縮小するヒステリシス（同じ入力でも直前の状態によって挙動が変わる仕組み。
ここでは枠が距離の揺らぎで細かく震える〈flicker〉のを防ぐ）付きの自動
追従です。

**3D**: ピンホールカメラ（レンズの歪みを考えない単純な透視投影モデル）が
表示範囲の中心を注視点として周回します。ドラッグで視点を回転、ホイールまた
はピンチ操作で距離を変更、ダブルクリックで既定の向き・全体が収まる距離へ
リセットします。床格子はカメラから15m以内は1m間隔、それより遠くは2m間隔
（偶数座標のみ延伸）で、地平線（床面上の無限遠が収束する行。カメラの向き
から解析的に求めます）まで伸び、遠いほど薄く表示されます。原点には右手系
の座標軸（x=赤・y=緑・z=青）を描きます。

### 4.4 アンカー一覧

登録済みアンカーごとに1行、色付きドット・距離（直近60秒・最大24点の
スパークライン付き）・成功率（起動からの累積、`stats` 行）・「1リンク」
（直近の測位周期でこのアンカーとの測距に要した時間、`fix.anchors[].elapsed_ms`）・
状態チップを表示します。状態チップは、無効化されていれば「無効」、
**3秒間新しい測距値が来なければ「未応答」**、それ以外は「有効」です。
**旧デザインにあった詳細表（試行数・成功数・成功率・再試行数・救済数・
初回成功率・現在の距離・所要時間・有効/無効・座標を1行にまとめた表）は
廃止され、この要約表示に置き換わりました。** 個々のカウンタが必要な場合は
生ログ（§4.8）の `stats`/`anchor_stats` 行、または無線コンソール（§5）の
`anchor list` を使ってください。

### 4.5 チャート（直近60秒）

2×2で並びます。上に色の凡例（初出順、最大8アンカーぶん。アンカー一覧・
測位図と共通の配色）が出ます。

| チャート | 内容 |
|---|---|
| 距離 `meas.d` | アンカーごとに色分けした折れ線。ポイントした位置で全アンカーの値を一覧できる十字カーソル＋ツールチップ付き |
| 成功率 `stats` | 全アンカー合算（太線=瞬時、細線=累積）。ツールチップなし |
| 周期 `fix.cycle_ms` | 折れ線 + 薄い帯（直近1秒間の最大値）。ツールチップなし |
| 位置 `lv3.p / fix.p` | x/y（3D測位中はzも）を軸ごとに色分け。太線=EKF解、細線=通常解。距離チャートと同様の十字カーソル＋ツールチップ付き |

### 4.6 測距統計・位置ジッタ

いずれもブラウザ側で計算し、パネル右上の「10s/60s」で3つとも集計窓を
まとめて切替えます。

**測距統計**（アンカーごと）: 件数・中央値・平均・標準偏差[mm]・最小・最大・
飛び値（自分を含む前後9点の中央値から0.25m超ずれた点の数と割合）。

**位置ジッタ**（通常解／EKFの2行）: 件数・x/y平均・x/y/z標準偏差[mm]
（3D測位でzが揃っているときのみzの標準偏差を算出）・移動量の中央値/最大
[mm]（「1周期移動量」＝連続する2つの解の間の水平距離）。

表の下のキャプションに、残差RMSの中央値・90パーセンタイル・最大[mm]、
解けなかった周期数、除外あり周期数を集計窓ごとに表示します。

### 4.7 ノード（折りたたみ）

自機と（タグなら）集約された全アンカーのカードです。**既定では折りたたま
れています**（クリックで展開）。空きヒープ・PHY 設定・PLL 粗調整コード・
方式（SS/DS）・破棄数・WebSocket クライアント数・UDP 送受信数を表示します。
アンカーのカードはさらに Poll/Response/Final/Result 数・再起動数/Final
タイムアウト数・RX エラー/TX 失敗数・各段階の成功率（Poll→Response/
Response→Final/Result率）・距離の平均±標準偏差も出します。**5 秒間新しい
行が来ないカードは薄く表示されます**（`stale`）。

### 4.8 生ログ（折りたたみ）

直近**200行**の生の JSON をそのまま表示します（既定では折りたたまれ、開い
ている間だけ描画を更新します。裏側での蓄積自体は開閉に関わらず続きます）。
`type` 欄で絞り込み可能です。

### 4.9 記録・保存

上部バーの **「● 記録開始」**を押すと、以後受信した行をブラウザのメモリに
溜め始めます（ブラウザ内で完結し、デバイス側にはファイルを持ちません。
ボタンは「● 記録停止」に変わります）。**「保存」**を押すと
`uwb_<機体名>_<日時>.jsonl`（1 行 1 JSON の **JSON Lines**
〈https://jsonlines.org/〉形式）としてブラウザのダウンロード先へ保存され
ます。記録を続けたままページを閉じると記録内容は失われるので、区切りの
良いところで保存してください。**記録開始・保存はライブモード専用で、
再生モード中はどちらも無効化されます。**
...
上部バーの「ライブ｜再生」で切替えます。**再生**にすると画面下端に再生
バーが現れ、以下を操作できます。

| 操作 | 内容 |
|---|---|
| ファイルを開く | ボタンクリック、または再生バーへのドラッグ＆ドロップで `.jsonl`（記録・保存〈§4.9〉で作ったファイル）を読み込む |
| 再生／一時停止 | ▶/❙❙ ボタン。キーボードの Space でも切替可（入力欄にフォーカスがあるときは無効） |
| 再生速度 | 0.5×/1×/2×/4×/10× |
| シークバー | ドラッグで任意の位置へ。キーボードの ←/→ で1秒、Shift+←/→ で10秒移動 |
| seq表示 | 現在の行番号/総行数、再生中の行/秒 |
| ライブに戻る | 再生を終了し、ライブ受信へ戻る |

各行に含まれる `t`（起動からの経過秒。§8.1）を相対時刻として使い、ファイル
内で最初に `t` を持つ行を0秒として正規化します（`t` を持たない行は待たず
にその位置で流します）。**測位図の軌跡・チャートの表示窓・ノードカードの
`stale` 判定など、時間に依存する処理はすべて再生クロック（ファイル内の
相対時刻）を基準に動くため、シーク位置を変えるとその瞬間の状態がそのまま
再現されます。** ライブの WebSocket 接続は再生中も張ったままですが、届いた
行はすべて無視されます（「ライブに戻る」を押した時点で接続し直す必要が
ありません）。

URL のクエリで `?view=3d` を付けると測位図が最初から3D表示で開きます
（`?demo=1` は従来どおり、実機なしで表示確認するための擬似データ生成です）。

### 4.11 配色モード切替

上部バー右端のボタン（◐/☀/☾）は「システム」→「ライト」→「ダーク」の順に
循環します。選択はブラウザの `localStorage`（ブラウザ内に保存される設定
領域。このページを開いたブラウザだけで有効）のキー `uwbDashTheme` に保存
され、次回開いたときも引き継がれます。既定の「システム」では OS 側の配色
設定（ダークモード/ライトモード）にそのまま追従します。

### ブラウザの更新が止まる問題（2026-09-02 修正）

**症状**: ダッシュボードをしばらく開いていると更新が止まる、上部バーの受信レート
（行/秒）が不安定になる、ページを再読み込みすると一時的に直る、といった不具合が
ありました。原因調査時、`wifi`（§3）の `ws clients`（WebSocket 接続数）が、
実際に開いているブラウザの台数より多いまま減らない現象（例: ブラウザを 1 つしか
開いていないのに `ws clients : 5`）が手がかりになりました。

**原因**: ブラウザを閉じる・再読み込みすると WebSocket 接続は切れますが、機体側は
それを検知できず 50ms ごとに配信を続けます。すると相手側から **RST**（接続の強制切断通知）
が返り、機体側は次の受信でエラー（errno 104 または 128）を受け取るはずでした。
ところが使用している ESP-IDF v5.5.2 の内部関数は、このエラー値（負の数）を
`size_t`（0 以上しか表せない整数型）と比較しており、「ソケットが壊れていたら
切断扱いにする」判定が働きません。結果としてアプリ側はフレームを読めないのに
「切断しない」と応答し、この接続は**切断後も残った接続（残留接続）**として残り続けます。
残留接続は配信のたびに同じ失敗を繰り返す**ビジーループ**（休みなく同じ失敗を繰り返す状態。
実機で 26 秒間に 1.9MB の警告ログが出た例あり）を起こして全員への配信をカクつかせ、
さらに失敗のたびにこの接続の **LRU**（最も長く使われていない接続を追い出す仕組み）の
使用時刻だけが更新されるため、実際に見ている（受信専用で送信をしない）ブラウザの
方が「長く使っていない」と誤判定され、接続数の上限（6）に達すると**そちらが
追い出される**という逆転が起きていました。再読み込みのたびに残留接続が 1 つ増えます。

**修正**（`components/uwb_net/src/uwb_net_http.cpp`）:
1. コマンド処理でフレーム読み取りに失敗したら「切断」を返すよう変更（サーバが
   そのセッションを削除するようになった）。
2. 配信処理で、送信に失敗した接続は明示的に切断し、成功した接続（受信専用の
   ダッシュボードも含む）は使用時刻を更新するよう変更。
3. **キープアライブ**（無応答が続いたら自動で切る仕組み）を有効化（無通信 5 秒で
   チェック開始・5 秒間隔・3 回失敗で切断）。FIN（正常切断の合図）無しで消えた
   相手側も約 20 秒で自動的に片付くようになった。

比較の不具合そのものは ESP-IDF 側の問題で、今回はアプリ側のハンドラで回避しています
（ESP-IDF 本体は変更していません）。**確認方法**: `wifi`（§3）の `ws clients` が
実際に開いているダッシュボードの数と一致していれば正常です。再読み込み後は
1 秒程度で残留接続が消え、機器ごとネットワークから消えた場合でもキープアライブに
より約 20 秒で片付きます。
...
受け取ったコマンドをこのワーカーへ渡すだけで、自分自身では実行しません。
...
もう 1 台のデバイス（例: アンカー）の USB コンソールから、確認したい相手側
（例: タグの IP）を狙って打ちます。実行すると、そのデバイス自身がクライアントに
なって相手側の HTTP/TCP/WebSocket を素のソケットで呼び出し、結果を（打った側の）
...
> 相手側だと TCP の OS 側リトライ〈数十秒〉まで固まっていた問題を、非ブロッキング
> `connect()` + `select()` で修正済み）。相手側が電源断・IP 間違い等で応答しない
...
### `info` / `anchor list` の実機確認（コンソールの処理が回ってこない不具合の修正後）
...
- ダッシュボードページの JavaScript（チャート描画・アンカー一覧・コンソールの
  ...
  実際に採取したログを再生するモックサーバを使って動作確認済み**です。

### 実際の PC のブラウザで確認できたこと（2026-08-31、構成 A・ルーター経由）

- **`http://uwb-tag.local/`（および IP 直打ち）でダッシュボードが開き、
  ヘッダ・ノードカード・アンカー一覧・チャート・測位図が実データで更新される
  ことを、実機 2 台（タグ + アンカー）に対して PC の実ブラウザで確認済み**。
- **PC を繋がない給電のみ運用（充電器/モバイルバッテリ）でも同様に表示される
  ことを確認済み**（§13 の凍結不具合の修正後）。
- 実表示で「測位図にアンカー点・測距円が出ない」不具合が見つかり、`anchors` 行の
  周期再送（`6f6dea9`、§8.1）で修正した。修正後は WebSocket 接続から 1 秒以内に
  座標行が届くことをデータ側で確認済み。
...
- ページ内コンソールから `anchor list` 等を打って応答が返ること
  （TCP/WebSocket のコマンド往復自体は §6 の `net probe` で確認済み）
...
- [x] `http://uwb-tag.local/` （または IP 直打ち）でダッシュボードが開くこと（2026-08-31）
- [x] ヘッダ・ノードカード・アンカー一覧・チャート・測位図が実データで更新されること（2026-08-31）
- [ ] 「記録開始」→「保存」で `.jsonl` が実際にダウンロードされ、中身が読めること
- [ ] ページ内コンソールから `anchor list` 等を打って応答が返ること
- [ ] 複数ブラウザタブを同時に開いても壊れないこと
...
| 種別 | 出どころ | 頻度 | 内容 |
|---|---|---|---|
| `anchors` | タグ | 起動時・表が変わるたび・`stats` と同じ周期（既定 1 秒）で再送 | 登録済みアンカーの一覧（座標・アンテナ遅延・有効/無効）。後から接続したブラウザにも座標を届けるための再送で、この行が無いと測位図が描けない |
...
| **UDP ブロードキャスト**（`255.255.255.255:5006`、既定） | アンカー→タグ。アンカーは間引き後の行を 1400 バイト以内・行単位でブロードキャストし、タグは同じポートで受信して自局の配信へ混ぜる |
...
| **n07**（処理が回ってこない状態〈スタベーション〉への対策の暫定版＝1tick 版） | — | **99.3%**（5554 周期） | 8ms / 18ms / 33ms | （未記録） | 0 | （未記録） | （未記録） |
...
**n07 は §0-D 項目8の、処理が回ってこない問題への対策の暫定版（1 tick だけ譲る版）で測ったものです。**
実機で core 0 に固定された TCP コンソール・WebSocket ワーカーが依然として処理が回ってこないことが
...
- **`output off`（タグのみ）は USB だけでなく WebSocket/UDP への配信も止めます。**
  ...
  だけでなくダッシュボードのチャート・アンカー一覧・測位図も更新が止まります。
  **`node` 行だけは例外**で、`uwb_net_tx` タスクが 1 秒ごとに独立して組み立てて
  USB と配信の両方へ出す（`uwb_net_sink.cpp`）ため、`output off` 中でも
  ノード欄（§4.7、折りたたまれていても裏側では更新され続けます）のカード
  （機体情報・空きヒープ・Wi-Fi 状態）は動き続けます。
...
- **598525c より前のファームには、PC を繋がない給電のみ（充電器/モバイルバッテリ、
  USB ホスト無し）だと Wi-Fi が一切上がらない不具合がありました**（原因はコンソール
  入力タスクの busy loop。USB ホスト不在時に read が即エラーを返し、待ちなしの
  ループで Wi-Fi 起動タスクに処理が回ってきませんでした）。**最新ファームでは修正済みです。**
  給電のみ運用で無応答になったときは、タグを PC に挿して `bootlog` コマンドを
  打つと、前回どの段階まで起動したかを確認できます（凡例
  `0=entry 1=console 2=net_start 3=net_ok 4=got_ip`、直近 16 回分の履歴、
  起動 8 秒後に前回記録を自動再表示）。**`bootlog` はタグ専用で、アンカーには
  未実装です。**
