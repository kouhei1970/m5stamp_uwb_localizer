...
- 注意: Stamp S3 では IRQ 配線済み（G7）。**StampFly の GROVE 2系統案では IRQ 線が無い**
...
- **対処**: 本当にチャネル別プロファイルを持つ（ch5/ch9 の公式値を入れる）か、削除して正直に書く
  → M5Stamp UWB Module with FPC は **ch9 固定**なので、ch5 を捨てて ch9 専用にするのが最も正直
...
### 【R10】ch9 の PLL 再校正 — **M5Stamp UWB Module with FPC は ch9 固定なので必須**
