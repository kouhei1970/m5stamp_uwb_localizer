...
> - M5Stamp UWB Module は新しい製品で、コミュニティの作例もほとんどありません。
...
| **M5 AtomS3** | **5** | アンカー（固定局）のホスト **AtomS3R（現行）/ 無印 AtomS3（在庫限り）のどちらでも可**（IMU/地磁気を使わないため互換と推定。**実機未検証**） |
...
> **【訂正 2026-08-22】** 旧 `BRINGUP.md`（本節に統合済み）は
...
|---|:-:|:-:|---|
| GND | 1 | 7 | グランド |
| **VCC_3V3** | **2** | **1, 2** | **公式仕様 3.3V。チップ直結、絶対最大定格 4.0V。5V は絶対に入れない** |
| DW_WAKEUP | 3 | 5 | ウェイクアップ制御（省電力を使わないなら未配線可） |
| DW_IRQ | 4 | 4 | 割り込み出力（現状の実装はポーリングなので未配線でも動く） |
| DW_GP7 | 5 | 3 | 予備。**配線不要** |
| DW_RSTn | 6 | 6 | リセット（負論理）。**配線推奨** |
| DW_CDO | 7 | 8 | SPI **MISO** |
| GND | 8 | — | グランド（FPC 側には無い） |
| DW_CDI | 9 | 9 | SPI **MOSI** |
| DW_CSn | 10 | 10 | SPI **CS** |
| DW_CLK | 11 | 12 | SPI **SCK** |
| GND | 12 | 11 | グランド |
...
M5StampS3A は GPIO（General Purpose Input/Output、汎用入出力）に余裕があるので**フル配線**します。
...
> **IRQ（Interrupt ReQuest、割り込み要求）は pin 4 → G2 に必ず配線してください。**
> アンカー側の IRQ 経路は**実装済み**で、有効にすると測位レートが 31 Hz → 59 Hz に上がります
> （効くのは**レスポンダ側＝アンカー＝AtomS3 の折り返し時間**です）。
> **ここが配線されていないと後戻りできません。**
...
> **RSTn に外付けプルアップは付けないこと（2026-08-21 訂正）**: 以前の版はここに
> 「pin 6 (RSTn) ↔ pin 2 (3V3) 間に 10kΩ プルアップ」を保険として挙げていましたが、
> QM33120W データシートは RSTn について「Must not be pulled high by the external
> source（外部から High へプルしてはならない）」と明記しており、High 側は
...
| 症状 | 最初に疑う | 次に疑う |
|---|---|---|
| **Device ID が読めない** | 配線の向き・浮き（[4.3](#probe-troubleshoot)） | 配線長・GND の戻り経路。パスコン追加（[3.7](#extra-parts)） |
...
| 測位パイプラインと設定シリアライズのホスト検証（合成データ 188 件） | 実機での測距 |
...
| 項目 | 状況 |
|---|---|
| **半田パッドの向き（pin 1 の位置）** | **未確認（本リポジトリでは）。** 公式 PINMAP 画像が入手できていない。外部報告（公式 Arduino ライブラリ + 別 MCU）とは整合している（[3.1](#orientation)）が、[3.1](#orientation) の導通試験は必ず自分の個体で実施すること |
| 基板シルクにピン番号の印刷があるか | 未確認 |
| **DW_RSTn の High 側の駆動** | データシート上は IC 内蔵の POR 回路が駆動する設計（外部プルアップ禁止。2026-08-21 訂正、[3.7](#extra-parts)）。本リポジトリの実装（Hi-Z + リセット時のみ Low）はこれに沿っている。回路図が未入手のため実機波形は未確認 |
| VCC_3V3 直近のバイパスコンデンサ | 未確認 |
| `boards/*.h` のホスト側ピン番号 | **暫定値。** 現物のシルクと照合すること。**パッド番号↔信号の対応と向きは外部報告と整合**（[3.1](#orientation)）だが、ホスト側 GPIO 番号は本リポジトリ独自の暫定値なので別途照合が要る |
