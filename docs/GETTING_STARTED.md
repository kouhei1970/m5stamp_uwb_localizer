...
M5Stack **Stamp UWB-F** を 6 個買った人が、**タグ 1 台 + アンカー 5 台**の
...
> - Stamp UWB-F は新しい製品で、コミュニティの作例もほとんどありません。
...
| 品目 | 数量 | 備考 |
|---|---:|---|
| **M5Stack Stamp UWB-F**（SKU `S017-F`） | **6** | Qorvo QM33120W (DW3720) 搭載。無印 `S017` でも可 |
...
> 5V を入れると壊れます（`docs/SURVEY_stamp_uwb_f.md`）。
...
タグ役（Stamp S3 + Stamp UWB-F）とアンカー役（AtomS3 + Stamp UWB-F）を
...
**初期値は「一般的な typical 値」であって、Stamp UWB-F のアンテナ・RF 経路に
...
| [`SURVEY_stamp_uwb_f.md`](SURVEY_stamp_uwb_f.md) | モジュールのハードウェア仕様調査 |
