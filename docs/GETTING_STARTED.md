...
> - **配線の向き（標準構成では FPC ケーブルの挿し方向、半田パッド直付けならピン 1 の位置）は
>   実物で確認が必要です。** ここを間違えると電源逆接でモジュールが壊れます
>   （→ [3.1](#orientation)）。
...
| `boards/*.h` のホスト側ピン番号 | **`boards/stamps3.h` の SPI 4 本（SCK=G12 / MOSI=G11 / MISO=G13 / CS=G10）は実機で確認済み**（2026-08-27。値を変えずに PASS）。**同じファイルでも RST=G6 / IRQ=G7 / WAKEUP=G8 は未確認**（[4.2.1](#probe-result)）。`boards/atoms3.h` は全ピン未確認 |
| **RSTn / IRQ / WAKEUP が実際に効いているか** | **未確認。** `firmware/probe` が PASS してもこの 3 本は裏付けられない（理由は [4.2.1](#probe-result)）。RSTn は [5](#twr) 以降の温間リセットで、IRQ は [`EXPERIMENT_PLAN.md`](EXPERIMENT_PLAN.md) 実験7 で確認する |
