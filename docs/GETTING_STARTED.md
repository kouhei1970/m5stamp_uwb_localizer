...
検証しています（**2D 測位・3D 測位そのものは実機でも確認済み**〈アンカー3台・4台、
2026-09-02〜05、詳細は [`docs/HANDOFF.md`](HANDOFF.md) §1〉）。設定の永続化（[6.2](#anchors5-console) / [7.3](#survey)）についても、
...
タグと（UDP 集約された）全アンカーの数値・グラフ・測位図・無線コンソールが
...
**1 対 1 測距（SS-TWR・DS-TWR）・ブラウザダッシュボード・2D 測位（アンカー3台）・
3D 測位（アンカー4台）までは実機で動作確認済みです**（測距は M5StampS3A ×2、
タグ 1 台 + アンカー 1 台の構成で 2026-08-27〜31。2D/3D 測位はアンカー3〜4台構成で
2026-09-02〜05。詳細は [`docs/HANDOFF.md`](HANDOFF.md) §1）。**一方、自動測量の
無線側（アンカー間の自動測距）・アンテナ遅延校正・WAKEUP 配線はまだ実機で確認して
いません。**

| 検証済み | 未検証 |
|---|---|
| 全ファームのビルド（警告 0・エラー 0） | 自動測量の無線側（アンカー間の自動測距）。ソルバ自体は `survey` コマンドへの手入力〈巻尺実測3距離〉で実機確認済み（[11.5](#limitations)） |
| **実機での Device ID 読み出し**（2026-08-27） | アンテナ遅延校正（[9](#antenna-delay)） |
| **実機でのフレーム送受信 devtest**（2026-08-29） | WAKEUP 配線（[11.2](#limitations)。RSTn・16MHz SPI は 2026-08-29 の拡張 probe で確認済み） |
| **実機での SS-TWR 測距**（2026-08-29、850kbps・即時再試行2回・約1m・300秒2,180サイクルでサイクル成功率99.95%） | 5 アンカー・6 アンカー構成での更新レート実測（4 アンカーまでは実測済み、[11.4](#limitations)） |
| **実機での DS-TWR 測距**（2026-08-30、アンカー受信常時ONのv2アーキテクチャ、本番既定構成850kbps・IRQ有効・再試行2回で周期成功率99.5〜99.6%） | |
| 測位パイプラインと設定シリアライズのホスト検証（合成データ 200 件） | |
| 測位ソルバ（uwb_loc）のホストテスト | |
| ESP32-S3 上でのソルバ計算時間ベンチ | |
| **Wi-Fi 設定コンソール・ブラウザダッシュボード実表示・PC なし給電運用・Wi-Fi 併用時 DS-TWR 99.3〜99.4%**（いずれも2026-08-31） | |
| **アンカー 2 台目以降の UDP 集約**（2026-08-31） | |
| **`uwb_cfgstore` のアンカー座標・アドレス設定コンソールの実機読み書き**（`anchor set`/`addr set`+NVS保存、2026-09-02・09-04） | |
| **2D 測位**（アンカー3台、2026-09-02。残差RMS 0.05m） | |
| **3D 測位**（アンカー4台、2026-09-04〜05。真値照合で0.14〜0.39mの差） | |
...
| 更新レート | DS-TWR を**逐次**で回すので、1 周 = 1 リンク時間 × 台数。**アンカー4台では実機で周期35ms（1リンク8ms）を確認済み**（2026-09-04〜05）。5台以上での実測はまだ（[5.4](#twr-measure)） |

### 11.5 設定の永続化（NVS + コンソール）

`components/uwb_cfgstore/`（アンカー座標・アドレスの設定、[6.2](#anchors5-console),
[7.3](#survey)）は、**`anchor set` / `addr set` による設定と NVS への保存・起動時の
読み出しが実機で確認済みです**（2026-09-02・09-04。2D 測位・3D 測位の座標登録に
実際に使用。詳細は [`docs/HANDOFF.md`](HANDOFF.md) §1）。**Wi-Fi 設定の NVS（`uwb_net` 側、
[8.7](#net-dashboard)）は別の NVS 領域で、2026-08-31 に別途実機確認済みです。**
