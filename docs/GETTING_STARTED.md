...
> ### 先に読んでほしいこと（正直な現状）
>
> - **実機で確認済みなのはタグ 1 台 + アンカー 1 台の 1 対 1 測距までです**
>   （2026-08-27〜31。SPI 疎通 → SS-TWR サイクル成功率 99.95% → DS-TWR 周期成功率
>   99.5〜99.6%〈いずれも距離約 1m〉→ Wi-Fi ブラウザダッシュボード表示・PC を
>   繋がない給電まで）。**アンカー 5 台での 3D 測位（本書のゴール、§6 以降）は
>   まだ実機で動かしていません**（ホスト（PC）上の合成データでの検証のみ。
>   → [§11](#limitations)）。
...
>   （→ [3.1](#orientation)）。
> - つまり本書のうち §4（疎通確認）〜§5（1 対 1 測距）に相当する部分は実機で
>   通過済みですが、**§6 以降（アンカー 5 台・測位・校正）は設計資料から起こした
>   最良の推定**のままです。実物と違う点が出たら報告してください。
