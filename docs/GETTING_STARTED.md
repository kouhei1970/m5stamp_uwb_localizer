...
**標準構成（経路A、[`WIRING.md` §0.1](WIRING.md#route)）は、M5StampS3A + StampS3 BreakOut を
...
> はそのまま使えます。AtomS3 を使う場合は半田パッド（経路B、[`WIRING.md` §3.2](WIRING.md)）
...
> **経路B（半田パッド直付け）を選ぶ場合**は、代わりに次の道具が要ります（旧来の一覧。削除していません）。
...
本章では要約とリンクに留めます（同じ表が複数文書に散らばって片方だけ訂正から
取り残される事故が実際に起きたため）。

### 接続経路は3つある。まず経路を決める

[`WIRING.md` §0.1](WIRING.md#route) に詳しい比較表があります。要約すると:
...
> （上の「接続経路は3つある」および [`WIRING.md` §2](WIRING.md) と同じ結論）。

**半田パッドで配線するなら「パッド」列、FPC ケーブルで配線するなら「FPC」列の番号を使う。**
**両者を混ぜると電源が信号線に入ってモジュールが壊れる。**

**→ パッド番号 ↔ FPC 番号の対応表は [`WIRING.md` §2](WIRING.md) にあります（唯一の正本）。**

同じ表を複数の文書に置くと、訂正が片方だけに入って食い違う。実際にそれが起きたのが
上の訂正ボックスの件なので、番号の対応表は本書には置きません。覚えておくべき要点は3つ:
...
幸い FPC 側には**非対称性**があります（[`WIRING.md` §2](WIRING.md)）:
...
詳細な手順・根拠は [`WIRING.md` §4.0「向きを確定する」](WIRING.md) を参照してください。
...
**タグもアンカーも同じ配線になりました**（旧版はタグ 10 本・アンカー 9 本で別々でしたが、
経路Aではホストが両方とも M5StampS3A + StampS3 BreakOut になったため統一されています）。

**下表は FPC 番号で書いています。半田パッド番号ではありません。** パッド↔FPC の
対応表は [`WIRING.md` §2](WIRING.md) を参照してください。**パッド番号をそのまま
流用すると電源が信号線に入って壊れます。**

| FPC | 信号 | M5StampS3A | 必須 | 備考 |
|----:|------|------------|:----:|---|
| 1, 2 | VCC_3V3 | **3V3** | ● | **公式仕様 3.3V。絶対最大定格 4.0V。5V は絶対に入れない**（[WIRING.md §5.4](WIRING.md)） |
| 3 | DW_GP7 | — | | 配線不要 |
...
（AtomS3 では pin 4 → G2、`WIRING.md` §3.2）。

#### AtomS3 を代替に使う場合

AtomS3 を持っているなら、経路B（半田パッド、[`WIRING.md` §3.2](WIRING.md)）の配線で
...
- **FPC ケーブルをモジュールの PCB アンテナの下に通さない。** 公式も
  ...
  （[`WIRING.md` §1.5](WIRING.md)）。半田付けの飛び線と同じ理由で、測距距離に直接効きます。
...
（[`WIRING.md` §4.0「信号品質」](WIRING.md) より）。
...
一通り入っています（`WIRING.md` §4.5(a)）。上記はそれでも足りないときの保険です。
...
| L1 は OK だが L2 が FAIL | `dwt_probe()` の wakeup シーケンス／SPI の再現性 | RSTn へのプルアップは付けない（DS で `Must not be pulled high` と禁止されている。[3.6](#extra-parts) からプルアップの推奨は削除済み）。配線長・GND 戻り経路を見直す |
...
> 配線は経路B（半田パッド、[`WIRING.md` §3.2](WIRING.md)）になります。
...
| **DW_RSTn の High 側の駆動** | データシート上は IC 内蔵の POR 回路が駆動する設計（外部プルアップ禁止。2026-08-21 訂正、[3.6](#extra-parts)）。本リポジトリの実装（Hi-Z + リセット時のみ Low）はこれに沿っている。回路図が未入手のため実機波形は未確認 |
...
| **`S017-F` にキャステレーションパッドがあるか** | 公式の製品比較表では「`S017` と `S017-F` の差は FPC コネクタ実装の有無とケーブル同梱の有無だけ」で半田パッドは両方にあるとされている（`WIRING.md` §0）が、**実機での確認はまだ** |
