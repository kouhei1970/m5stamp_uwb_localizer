...
（`SF/tasks/tasks.cpp:44-57` のコメントに、コア1へ優先度22を同居させた際に数十秒にわたり
処理が回ってこない状態（スタベーション）が発生した実機バグの記録あり）。
...
`SF/components/sf_estimator_eskf/eskf_estimator.cpp:62-68` に「uint32_t は約71.6分で桁あふれして0に戻る」旨の記述あり。
...
**→ レートは統合のボトルネックではない。ボトルネックは精度とバイアス。**
...
3. **密結合（案A）にしてそれぞれの測距を個別の時刻で融合する** → 原理的に解決（§4.3）
...
| 精度 | **最良**。1本でも観測が入れば拘束になる（3本以下の周も使える）。各測距をそれぞれの時刻で融合できるので**周内スミアが原理的に消える** |
...
しか無い。これを定期的に呼ぶ、あるいは `pos_` を直接書き換えるという方式。
...
|---:|---|---|
| 24〜23 | コア1 専用帯。使わない | — |
| 22 | StateTask（安全 FSM）と同格 | **禁止**。安全 FSM の処理が回らなくなりうる |
...
| 2 | **段階的な再収束**。ゲートを超え続けたら一度で飛ばさず、`inflateCovariance(POS_X\|POS_Y)`（`eskf_core.hpp:173`、`eskf_core.cpp:123-160`）で共分散だけ膨らませ、ESKF に自然に引き寄せさせる |
